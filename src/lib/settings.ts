...
export function migrateRenamedPluginIds(settings: PluginSettings): PluginSettings {
  const mapId = (id: string) => RENAMED_PLUGIN_IDS[id] ?? id;
  return {
    order: Array.from(new Set(settings.order.map(mapId))),
    disabled: Array.from(new Set(settings.disabled.map(mapId))),
  };
}
