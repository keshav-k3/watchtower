...
- Use the packaged CLI first: `Watchtower.app/Contents/Helpers/CodexBarCLI`.
- Do not use `Watchtower.app/Contents/MacOS/codexbar`; that is the app binary and may appear to hang as a CLI.
...
```bash
Watchtower.app/Contents/Helpers/CodexBarCLI config validate
stat -f '%Lp %N' "$HOME/.watchtower/config.json"
```
...
jq '(.providers // []) |= map(.apiKey = (if .apiKey then "<redacted>" else .apiKey end) |
  ...
  "$HOME/.watchtower/config.json"
```

Before editing config, make a backup:

```bash
cp "$HOME/.watchtower/config.json" "$HOME/.watchtower/config.pre-qa-$(date +%Y%m%d%H%M%S).json"
chmod 600 "$HOME/.watchtower"/config.pre-qa-*.json
```
...
```bash
pkill -x CodexBar || pkill -f 'Watchtower.app/Contents/MacOS/CodexBar' || true
open -n "$PWD/Watchtower.app"
