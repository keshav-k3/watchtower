...
Use this when Watchtower is not working and you need to share debug info.
...
1. In your macOS menu bar, find the Watchtower icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If Watchtower does not open at all, skip this step and continue.
...
```text
~/Library/Logs/com.keshavk3.watchtower
```
...
1. Attach `watchtower.log`.
2. If you also see files like `watchtower.log.1`, attach those too.
