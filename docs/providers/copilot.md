...
1. **Watchtower Keychain** (`Watchtower-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the Watchtower keychain for faster access on subsequent probes.
