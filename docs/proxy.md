...
Watchtower can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.watchtower/config.json`
- Default: off
- UI: none

## Config File

Create `~/.watchtower/config.json`:
...
- Restart Watchtower after changing the file.
...
This applies to provider and plugin HTTP requests that go through Watchtower's built-in HTTP client.
