(function () {
  const KEYCHAIN_SERVICE = "Watchtower-copilot";
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.token) {
          ctx.host.log.info("token loaded from Watchtower keychain");
          return { token: parsed.token, source: "keychain" };
        }
      }
    } catch (e) {
      ctx.host.log.info("Watchtower keychain read failed: " + String(e));
    }
    return null;
  }
  ...
  function probe(ctx) {
    ...
    // Persist gh-cli token to Watchtower keychain for future use
    if (source === "gh-cli") {
      saveToken(ctx, token);
    }
  ...
  globalThis.__watchtower_plugin = { id: "copilot", probe };
})();
