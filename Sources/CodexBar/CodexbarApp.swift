...
struct CodexBarApp: App {
    ...
    init() {
        ...
        let level = CodexBarLog.parseLevel(env["WATCHTOWER_LOG_LEVEL"] ?? env["CODEXBAR_LOG_LEVEL"]) ?? storedLevel
        CodexBarLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.keshavk3.watchtower"),
            level: level,
            json: false))

        let version = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "unknown"
        let build = Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as? String ?? "unknown"
        let gitCommit = Bundle.main.object(forInfoDictionaryKey: "WatchtowerGitCommit") as? String ?? "unknown"
        let buildTimestamp = Bundle.main.object(
            forInfoDictionaryKey: "WatchtowerBuildTimestamp") as? String ?? "unknown"
