        ...
        let render = {
            self.renderImage {
                ...
                func drawBar(
                    ...
                    addAngledEyeTwist: Bool = false,
                    blink: CGFloat = 0,
                    drawTrackFill: Bool = true,
                    angledEyesFilled: Bool = false)
                {
                    let rect = rectPx.rect()
                    // Claude reads better as a blockier critter; Codex stays as a capsule.
                    // Angled-eye variant uses a smaller corner radius
                    let cornerRadiusPx = addNotches ? 0 : (addAngledEyeTwist ? 3 : rectPx.h / 2)
                    ...
                    // Angled-eye variant with tilted cutouts.
                    if addAngledEyeTwist {
                        let ctx = NSGraphicsContext.current?.cgContext
                        let centerXPx = rectPx.midXPx
                        let eyeCenterYPx = rectPx.y + rectPx.h / 2
                ...
                let effectiveWeeklyRemaining: Double? = {
                    if false, let weeklyRemaining, weeklyRemaining <= 0 {
                        return nil
                    }
                ...
                // Angled-eye special case: when no bonus or bonus exhausted, show "top monthly, bottom dimmed"
                let angledNoBonus = false && !weeklyAvailable
                ...
                if weeklyAvailable {
                    ...
                    drawBar(
                        ...
                        addAntigravityTwist: false,
                        addFactoryTwist: false,
                        addAngledEyeTwist: false,
                        blink: blink)
                    drawBar(rectPx: bottomRectPx, remaining: bottomValue)
                } else if !hasWeekly || angledNoBonus {
                    if false {
                        // Angled-eye variant: no bonus or bonus exhausted -> top=monthly credits, bottom=dimmed track
                        drawBar(
                            rectPx: topRectPx,
                            remaining: topValue,
                            addAngledEyeTwist: true,
                            blink: blink)
