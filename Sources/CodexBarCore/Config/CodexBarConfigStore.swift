...
public enum CodexBarConfigStoreError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .invalidURL:
            "Invalid Watchtower config path."
        case let .decodeFailed(details):
            "Failed to decode Watchtower config: \(details)"
        case let .encodeFailed(details):
            "Failed to encode Watchtower config: \(details)"
        }
    }
}

public struct CodexBarConfigStore: @unchecked Sendable {
    public static let pathEnvironmentKey = "WATCHTOWER_CONFIG"
    ...
    {
        let configOverride = environment[pathEnvironmentKey] ?? environment["CODEXBAR_CONFIG"]
        if let override = configOverride?.trimmingCharacters(in: .whitespacesAndNewlines),
           !override.isEmpty
        {
