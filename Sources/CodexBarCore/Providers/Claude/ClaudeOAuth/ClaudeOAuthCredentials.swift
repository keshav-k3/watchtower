...
public enum ClaudeOAuthCredentialsStore {
    ...
    public static let environmentTokenKey = "WATCHTOWER_CLAUDE_OAUTH_TOKEN"
    public static let legacyEnvironmentTokenKey = "CODEXBAR_CLAUDE_OAUTH_TOKEN"
    public static let environmentScopesKey = "WATCHTOWER_CLAUDE_OAUTH_SCOPES"
    ...
    public static let environmentClientIDKey = "WATCHTOWER_CLAUDE_OAUTH_CLIENT_ID"
    ...
    private static var oauthClientID: String {
        ...
        return (environment[self.environmentClientIDKey] ?? environment[self.legacyEnvironmentClientIDKey])?
    ...
    private struct Repository {
        ...
        private func resolvedCacheOwner(_ owner: ClaudeOAuthCredentialOwner) -> ClaudeOAuthCredentialOwner {
            guard owner == .watchtower else { return owner }
        ...
        func shouldAttemptFreshnessSyncFromClaudeKeychain(cached: ClaudeOAuthCredentialRecord) -> Bool {
            ...
            case .onlyOnUserAction:
                if ProviderInteractionContext.current != .userInitiated {
                    if ProcessInfo.processInfo.environment["WATCHTOWER_DEBUG_CLAUDE_OAUTH_FLOW"] == "1" ||
    ...
    private static func saveRefreshedCredentialsToCache(_ credentials: ClaudeOAuthCredentials) {
        ...
        self.saveToCacheKeychain(jsonData, owner: .watchtower)
    ...
    {
        guard
            let token = (environment[self.environmentTokenKey] ?? environment[self.legacyEnvironmentTokenKey])?
                .trimmingCharacters(
                    in: .whitespacesAndNewlines),
                !token.isEmpty
        else {
        ...
        let scopes: [String] = {
            guard let raw = environment[self.environmentScopesKey] ?? environment[self.legacyEnvironmentScopesKey]
