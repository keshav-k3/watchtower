...
/// Proxy configuration loaded from ~/.watchtower/config.json
...
/// Config file path: ~/.watchtower/config.json
fn config_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".watchtower").join("config.json"))
}
