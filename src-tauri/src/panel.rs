...
tauri_panel! {
    ...
    panel_event!(WatchtowerPanelEventHandler {
...
pub fn init(app_handle: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    let panel = window.to_panel::<WatchtowerPanel>()?;
    ...
    let event_handler = WatchtowerPanelEventHandler::new();
