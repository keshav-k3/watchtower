...
fn current_macos_keychain_account_from_user_env(user_env: Option<String>) -> String {
    user_env
        ...
        .unwrap_or_else(|| "watchtower-user".to_string())
}
...
fn read_env_from_interactive_shell(program: &str, name: &str) -> Option<String> {
    const START_MARKER: &str = "__WATCHTOWER_ENV_START__";
    const END_MARKER: &str = "__WATCHTOWER_ENV_END__";
...
fn inject_http<'js>(
    ...
    ctx.eval::<(), _>(
        r#"
        (function() {
            // Will be patched after __watchtower_ctx is set.
            if (typeof __watchtower_ctx !== "undefined") {
                void 0;
            }
        })();
        "#
...
pub fn patch_http_wrapper(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var rawFn = __watchtower_ctx.host.http._requestRaw;
            __watchtower_ctx.host.http.request = function(req) {
...
/// Inject utility APIs (line builders, formatters, base64, jwt) onto __watchtower_ctx
pub fn inject_utils(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var ctx = __watchtower_ctx;
...
pub fn patch_ls_wrapper(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var rawFn = __watchtower_ctx.host.ls._discoverRaw;
            __watchtower_ctx.host.ls.discover = function(opts) {
                var optsJson;
...
pub fn patch_ccusage_wrapper(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var rawFn = __watchtower_ctx.host.ccusage._queryRaw;
            __watchtower_ctx.host.ccusage.query = function(opts) {
...
mod tests {
    ...
    fn extract_marked_value_ignores_noisy_shell_output() {
        let stdout = concat!(
            ...
            "__WATCHTOWER_ENV_START__\n",
            "  sk-test-key-12345  \n",
            "__WATCHTOWER_ENV_END__\n",
            "\u{1b}[32muser@host\u{1b}[0m\n"
        );
        let value =
            extract_marked_value(stdout, "__WATCHTOWER_ENV_START__", "__WATCHTOWER_ENV_END__");
        assert_eq!(value.as_deref(), Some("sk-test-key-12345"));
    }
    ...
    fn extract_marked_value_returns_none_when_marked_value_is_empty() {
        let stdout = "__WATCHTOWER_ENV_START__\n  \n__WATCHTOWER_ENV_END__\n";
        let value =
            extract_marked_value(stdout, "__WATCHTOWER_ENV_START__", "__WATCHTOWER_ENV_END__");
        assert!(value.is_none());
    }
    ...
    fn crypto_api_exposes_decrypt() {
        ...
        ctx.with(|ctx| {
            ...
            let globals = ctx.globals();
            let probe_ctx: Object = globals.get("__watchtower_ctx").expect("probe ctx");
    ...
    fn crypto_api_decrypts_node_generated_envelope_from_js() {
        ...
        ctx.with(|ctx| {
            ...
            inject_host_api(&ctx, "test", &app_data, "0.0.0").expect("inject host api");
            let js_expr = format!(
                r#"__watchtower_ctx.host.crypto.decryptAes256Gcm("{}", "{}")"#,
                envelope, key_b64
            );
    ...
    fn crypto_api_exposes_sha256_hex() {
        ...
        ctx.with(|ctx| {
            ...
            let result: String = ctx
                .eval(r#"__watchtower_ctx.host.crypto.sha256Hex("hello")"#)
                .expect("js sha256");
            assert_eq!(
                result,
            ...
            let empty: String = ctx
                .eval(r#"__watchtower_ctx.host.crypto.sha256Hex("")"#)
        ...
        ctx.with(|ctx| {
            ...
            let message: String = ctx
                .eval(
                    r#"
                    try {
                        __watchtower_ctx.host.keychain.readGenericPassword("__watchtower_missing_service__");
                        "ok";
                    } catch (e) {
                        String(e);
                    }
    ...
    fn env_api_respects_allowlist_in_host_and_js() {
        ...
        ctx.with(|ctx| {
            ...
            for name in WHITELISTED_ENV_VARS {
                ...
                let js_expr = format!(r#"__watchtower_ctx.host.env.get("{}")"#, name);
            ...
            let blocked: Option<String> = get
                .call(("__WATCHTOWER_TEST_NOT_WHITELISTED__".to_string(),))
            ...
            let js_blocked: Option<String> = ctx
                .eval(r#"__watchtower_ctx.host.env.get("__WATCHTOWER_TEST_NOT_WHITELISTED__")"#)
    ...
    fn env_api_prefers_process_env() {
        ...
        ctx.with(|ctx| {
            ...
            let js_value: Option<String> = ctx
                .eval(r#"__watchtower_ctx.host.env.get("ZAI_API_KEY")"#)
                .expect("js get");
            assert_eq!(
                js_value.as_deref(),
                Some("sk-process-env-test-1234567890"),
    ...
    fn current_macos_keychain_account_prefers_explicit_user_value() {
        assert_eq!(
            current_macos_keychain_account_from_user_env(Some("watchtower-test-user".to_string())),
            "watchtower-test-user"
        );
    }
    ...
    fn redact_body_redacts_name_field() {
        let body =
            r#"{"userStatus":{"name":"Keshav K","email":"rob@sunstory.com","planStatus":{}}}"#;
        let redacted = redact_body(body);
        assert!(
            !redacted.contains("Keshav K"),
        ...
        // "Keshav K" is 11 chars (<=12) so becomes [REDACTED]
        assert!(
    ...
    fn ccusage_path_entries_with_home_and_existing_path_preserves_order() {
        let home = std::path::PathBuf::from("/tmp/watchtower-home");
    ...
    fn nvm_default_bin_path_resolves_version_with_v_prefix() {
        let home = std::env::temp_dir().join("watchtower-test-nvm-v-prefix");
    ...
    fn nvm_default_bin_path_resolves_version_without_v_prefix() {
        let home = std::env::temp_dir().join("watchtower-test-nvm-no-v-prefix");
        let alias_dir = home.join(".nvm/alias");
        std::fs::create_dir_all(&alias_dir).expect("create alias dir");
    ...
    fn nvm_default_bin_path_returns_none_when_alias_missing() {
        let home = std::env::temp_dir().join("watchtower-test-nvm-no-alias");
        let _ = std::fs::remove_dir_all(&home);
        let result = nvm_default_bin_path(&home);
    ...
    fn ccusage_path_entries_with_includes_nvm_default_version() {
        let home = std::env::temp_dir().join("watchtower-test-nvm-entries");
        let alias_dir = home.join(".nvm/alias");
        std::fs::create_dir_all(&alias_dir).expect("create alias dir");
        std::fs::write(alias_dir.join("default"), "22.16.0").expect("write alias");
...
"#;
    ...
    fn ccusage_runner_retries_legacy_package_when_current_package_fails() {
        ...
        let test_id = format!(
            "watchtower-ccusage-legacy-fallback-{}",
