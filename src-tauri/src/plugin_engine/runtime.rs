...
fn run_probe_with_timeout(
    ...
    ctx.with(|ctx| {
        ...
        let plugin_obj: Object = match globals.get("__watchtower_plugin") {
            Ok(obj) => obj,
            Err(_) => return error_output(plugin, "missing __watchtower_plugin".to_string()),
        };
...
mod tests {
    ...
    fn temp_app_dir(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        std::env::temp_dir().join(format!("watchtower-test-{}-{}", label, nanos))
    }
