#!/usr/bin/env bash
# Reset Watchtower: kill running instances, build, package, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ROOT_DIR}/Watchtower.app"
APP_PROCESS_PATTERN="Watchtower.app/Contents/MacOS/Watchtower"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/.build/debug/Watchtower"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/.build/release/Watchtower"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/watchtower-compile-and-run-${LOCK_KEY}"
...
kill_all_codexbar() {
  ...
  pkill -9 -x "Watchtower" 2>/dev/null || true
  ...
  fail "Failed to kill all Watchtower instances."
}
...
# 2) Kill all running Watchtower instances (debug, release, bundled).
log "==> Killing existing Watchtower instances"
kill_all_codexbar
kill_claude_probes
...
if [[ "${SIGNING_MODE:-adhoc}" == "adhoc" && "${CLEAR_ADHOC_KEYCHAIN}" == "1" ]]; then
  log "==> Clearing Watchtower keychain entries (adhoc signing)"
  # Clear both the legacy keychain store and the current cache service when developers explicitly want a clean reset
  # of Watchtower-owned keychain state for ad-hoc builds.
  delete_keychain_service_items "com.keshavk3.watchtower"
  delete_keychain_service_items "com.keshavk3.watchtower.cache"
elif [[ "${SIGNING_MODE:-adhoc}" == "adhoc" ]]; then
  log "==> Preserving Watchtower keychain entries (pass --clear-adhoc-keychain to reset adhoc keychain state)"
fi
...
if [[ "${DEBUG_LLDB}" == "1" ]]; then
    run_step "package app" env WATCHTOWER_ALLOW_LLDB=1 "${PACKAGE_ENV[@]}" "${ROOT_DIR}/Scripts/package_app.sh" debug
else
  if [[ -n "${SIGNING_MODE}" ]]; then
    run_step "package app" env WATCHTOWER_SIGNING="${SIGNING_MODE}" "${PACKAGE_ENV[@]}" "${ROOT_DIR}/Scripts/package_app.sh"
  else
...
if ! open "${APP_BUNDLE}"; then
  ...
  "${APP_BUNDLE}/Contents/MacOS/Watchtower" >/dev/null 2>&1 &
  disown
fi
