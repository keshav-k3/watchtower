...
ZIP_NAME=$(watchtower_app_zip_name "$MARKETING_VERSION" "$ARCHES_VALUE")
DSYM_ZIP=$(watchtower_dsym_zip_name "$MARKETING_VERSION" "$ARCHES_VALUE")
...
if [[ -z "${APP_IDENTITY}" ]]; then
  echo "Missing APP_IDENTITY for Developer ID signing." >&2
  exit 1
fi

NOTARIZATION_TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/watchtower-notarize.XXXXXX")
chmod 700 "$NOTARIZATION_TEMP_DIR"
API_KEY_PATH="$NOTARIZATION_TEMP_DIR/watchtower-api-key.p8"
